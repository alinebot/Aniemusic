...
async def play(client: Client, message_: Message):
    ...
    res = await message_.reply_text("DenvilMusic=🔄 Processing...")

    if audio:
        if round(audio.duration / 60) > DURATION_LIMIT:
            raise DurationLimitError(
                f"Denvilmusic=Videos longer than {DURATION_LIMIT} minute(s) aren't allowed, the provided video is {audio.duration / 60} minute(s)"
            )
        ...
        if offset == None:
            await res.edit_text("DenvilMusic=❕ You did not give me anything to play.")
            return
